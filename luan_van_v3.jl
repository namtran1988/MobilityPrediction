...
test_user_count = 180;
feature_hashing_lenght = 100; # Số lượng dữ liệu muốn đưa về khi sử dụng feature hashing
...
# Lấy vector các điểm di chuyển của user góp lại thành dữ liệu để phân tích kmean
function run()
	set_max_min_config_from_data();
	data = zeros(maxLenghtTrajectory,test_user_count);
	for i = 1:test_user_count
		v = create_user_trajectory(i-1);
		for x in 1:maxLenghtTrajectory
			data[x,i] = v[x];
		end
	end
...
# Hàm cải tiến của hàm run(), Chạy phân tích dữ liệu
...
function run1()
	trajectoryLenght = maxLenghtTrajectory * maxLenghtTrajectory;
	set_max_min_config_from_data();
	data = zeros(trajectoryLenght,test_user_count);
	for i = 1:test_user_count
		v = create_user_trajectory(i-1);
		t = convert_to_trajectory_vector(v);
		for x in 1:trajectoryLenght
			data[x,i] = t[x];
		end
	end
